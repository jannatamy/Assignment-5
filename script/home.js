...
const displayIssue = (issues) => {
    ...
    issues.forEach(issue => {
        ...
        newDiv.innerHTML = `<div onclick ="loadModals(${issue.id})" class="max-w-sm bg-white border-0 border-t-4 mt-3 h-full flex flex-col justify-between rounded-xl shadow-sm overflow-hidden w-full ${issue.status === "open" ? 'border-green-500' : 'border-purple-600'} ">
                <div class="p-5">
                    ...
                    <div class="flex gap-2 mb-4">
                        ...
                        <span class="inline-flex items-center gap-1 px-3 py-1   text-sm font-medium rounded-full uppercase 
                            ${issue.labels[0] === 'enhancement' ? 'bg-green-50 text-green-500': 'bg-red-50 text-red-500'}">
                            ${issue.labels[0]}
                        </span>

                        <span class="inline-flex items-center gap-1 px-3 py-1  text-sm font-medium uppercase rounded-full
                            ${issue.labels[1] === 'enhancement' ? 'bg-green-50 text-green-500': ' text-orange-600'} ${issue.labels[1] ? 'border border-orange-100 bg-orange-50' : 'border-none'} ">
...
const displayModals = (issue) => {
    ...
    newDiv.innerHTML = `<div class="modal-box max-w-3xl">
            ...
            <h2 class="text-3xl font-bold text-slate-800 mb-2">Fix broken image uploads</h2>
                ...
                <div class="flex gap-2 mb-8">
                    ...
                    <p class=" border  px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1 
                        ${issue.labels[0]?.toLowerCase() === 'enhancement' ? 'bg-green-50 text-green-500': 'bg-red-50 text-red-500'}">
                        ${issue.labels[0]}
                    </p>
                    
                    <span class="bg-amber-50 text-amber-500 border border-amber-100 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1
                        ${issue.labels[1]?.toLowerCase() === 'enhancement' ? 'bg-green-50 text-green-500': 'bg-orange-50 text-orange-600'}">
                ...
                <div class="bg-slate-50 rounded-xl p-6 flex justify-between items-center mb-10">
                    ...
                    <div class="text-center ">
                        ...
                        <p class=" mt-1 px-5 py-1.5 rounded-sm text-sm font-bold
                            ${issue.priority === "high" ? 'bg-red-100 text-red-600' : 'text-[#e08f03] bg-amber-100'} "> ${issue.priority}
                        </p>
